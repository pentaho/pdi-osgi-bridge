...
public class KarafLifecycleListener implements IPhasedLifecycleListener<KettleLifecycleEvent> {
   ...
   * is only properly functioning with booting Karaf with empty caches. This is because of a behaviour (bug?) of the
   * Karaf Feature Service that when booting with full caches, it hydrates the persisted state where all features have
   * been installed and started. As such, when asking the Feature Service if a given feature is installed, it will
   ...
   * With Karaf empty caches this means wait until all bundles defined in the startup.properties have started.
   * With Karaf full caches this means wait until all cached bundles have started.
   */
